...
%     % including the effects of stall
%     sigma_alpha = (1+(vtol.e)^(-vtol.M*(alpha-vtol.alpha0))+(vtol.e)^(vtol.M*(alpha+vtol.alpha0)))...
%                     /((1+(vtol.e)^(-vtol.M*(alpha-vtol.alpha0)))*(1+(vtol.e)^(vtol.M*(alpha+vtol.alpha0))));
...
%     CDalpha = vtol.C_D_0 + vtol.C_D_alpha * abs(alpha); % RRP suggested to make it abs(alpha) because it should be sign agnostic
   ...
   CDalpha = vtol.C_D_p  + ((vtol.C_L_0 + vtol.C_L_alpha*abs(alpha))^2)/(pi*vtol.e*vtol.AR);
   sigma_alpha = (1 + exp(-vtol.M*(alpha-vtol.alpha0)) + exp(vtol.M*(alpha+vtol.alpha0)))/((1+exp(-vtol.M*(alpha-vtol.alpha0)))*(1+exp(vtol.M*(alpha+vtol.alpha0))));
   CLalpha = (1-sigma_alpha)*(vtol.C_L_0 + vtol.C_L_alpha*alpha) + sigma_alpha*(2*sign(alpha)*(sin(alpha)^2)*cos(alpha));
    ...
    C_Z_alpha = -CDalpha*sin(alpha) - CLalpha*cos(alpha);
...
%         fa_x = 0.5*rho*Va^2*S*(C_X_alpha + C_Xq_alpha*(vtol.c/(2*Va))*q + C_X_delta_e_alpha*delta_e);
    ...
    %fp_x = 0.5*rho*vtol.S_prop*C_prop*((k_motor*delta_t)^2-Va^2); %from
    %TEXTBOOK
    
    %Corrections from Errata
    ...
    Qp = rho*(vtol.D_prop)^5*vtol.C_Q0*Omega_p^2/(4*pi^2)+ ...
         rho*(vtol.D_prop)^4*vtol.C_Q1*Va*Omega_p/(2*pi) + ...
         rho*(vtol.D_prop)^3*vtol.C_Q2*Va^2;
