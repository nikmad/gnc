...
void guidance(float* guidance_commands)
{
	...
	*/
	...
	path_manager(in_pmanager, out_pmanager);
	...
	*/
	...
	path_follow(in_pfollow, out_pfollow);
...
void path_manager_dubins(float in[],int start_of_simulation,float waypoints[5][WAYPOINT_SIZE],float out[])
{
  float state[16];
  int NN = 0;
  int i,j,k;
...
void path_planner(float in[], float out[])
//void path_planner(float out[])
{
	float NN=0.0;
...
void path_manager(float in[],float out[])
{
	static int start_of_simulation;
	float  state[16];
	...
	else
	{
		...
		//{
		//	//printf("Testiugskfg3\n");
		//	path_manager_fillet(in,start_of_simulation, waypoints,out);
...
void path_follow(float in[], float out[])
{
