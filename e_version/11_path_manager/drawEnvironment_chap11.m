function drawEnvironment_chap11(uu,atp)
    ...
    if t==0,
       ...
       [Vertices,Faces,facecolors] = defineAircraftBody(scale);                              
        aircraft_handle = drawBody(Vertices,Faces,facecolors,...
                                   ...
                                   []);
        hold on
        waypoint_handle = drawWaypoints(waypoints, atp.R_min, []);
        ...
        ylabel('North')
        zlabel('-Down')
        axis([-S/5,S,-S/5,S,0,3*atp.map.MaxHeight]);
        view(00,90)  % set the view angle for figure
        grid on
    ...
    % at every other time step, redraw MAV
        ...
        drawBody(Vertices,Faces,facecolors,...
                     pn,pe,pd,phi,theta,psi,...
                     aircraft_handle);
        drawWaypoints(waypoints, atp.R_min, waypoint_handle);
