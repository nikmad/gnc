...
theta_c  = uu(3+NN);
chi_c    = uu(4+NN);  % commanded course (rad)
phi_c    = uu(5+NN);
...
elseif (h >= h_c-h_hold && h < h_c + h_hold)
    [delta_t, intg, diff, err] = airspeed_with_throttle(Va_c, Va, AP, vtol, intg_prev(4), diff_prev(4), err_prev(4));
    intg_prev(4) = intg;
    diff_prev(4) = diff;
    err_prev(4) = err;  
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [delta_t_sat, intg_new, diff_new, err_new] = airspeed_with_throttle(Va_c, Va, AP, vtol, intg, diff, err)
