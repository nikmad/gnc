...
%     e3dot = 0.5*(r*e0+q*e1-p*e2+0*e3);
   ...
   J_cost = lmbda*(1-e_mag^2);

    e0dot = 0.5*(J_cost*e0 - p*e1 - q*e2 - r*e3);
    e1dot = 0.5*(p*e0 + J_cost*e1 + r*e2 - q*e3);
    e2dot = 0.5*(q*e0 - r*e1 + J_cost*e2 + p*e3);
    e3dot = 0.5*(r*e0 + q*e1 - p*e2 + J_cost*e3);
...
function sys=mdlOutputs(t,x)
    ...
    vSmall = 10^-4;
%      
    %if (2*abs(e0*e1+e2*e3) < vSmall) && (abs(e0^2+e3^2-e1^2-e2^2) < vSmall)
    if (2*abs(e0*e1+e2*e3) < vSmall)
      y(7) = 0;
    else
